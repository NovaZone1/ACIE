#!/usr/bin/env python3
"""Aggregate five official LSTM seeds and compare them with MLP and ACIE ensembles."""
...
def main() -> None:
    ...
    summary = {"schema": "acie.official-fair-lstm-five-seed-summary.v1", "model": "lstm", "seeds": args.seeds, "aggregation": "arithmetic mean of per-seed probabilities", "directions": {}}

    for direction_index, direction in enumerate(DIRECTIONS):
        paths = [official_root / direction / "lstm" / f"seed{seed}" / "predictions.jsonl" for seed in args.seeds]
        ids, labels, scores, metadata, matrix = aligned_ensemble(paths)
        per_seed = []
        for seed, row_scores in zip(args.seeds, matrix, strict=True):
            metrics = json.loads((official_root / direction / "lstm" / f"seed{seed}" / "metrics.json").read_text(encoding="utf-8"))
            calculated = {"AP": float(average_precision_score(labels, row_scores)), "AUROC": float(roc_auc_score(labels, row_scores))}
            if metrics.get("model") != "lstm" or metrics.get("seed") != seed or not np.allclose([metrics["AP"], metrics["AUROC"]], [calculated["AP"], calculated["AUROC"]], rtol=0, atol=1e-12):
        ...
        ensemble_path = official_root / direction / "lstm" / "five_seed_ensemble_predictions.jsonl"
        ensemble_rows = []
        ...
        for comparison_index, (name, comparison_paths) in enumerate(comparison_sets.items()):
            ...
            if other_ids != ids or not np.array_equal(other_labels, labels):
                raise ValueError(f"LSTM/{name} sample mismatch for {direction}")
            comparisons[f"official_lstm_minus_{name}"] = bootstrap_difference(labels, scores, other_scores, [row["group_id"] for row in metadata], args.bootstrap, 20261014 + direction_index * 10 + comparison_index)
    ...
    output = official_root / "five_seed_lstm_summary.json"
