#!/usr/bin/env python3
"""Aggregate five official MLP seeds and compare them with frozen ACIE ensembles."""
...
def aligned_ensemble(paths: list[Path]) -> tuple[list[str], np.ndarray, np.ndarray, list[dict], np.ndarray]:
    ...
    return ids, labels, score_matrix.mean(axis=0), [tables[0][sample_id] for sample_id in ids], score_matrix
...
def bootstrap_difference(labels: np.ndarray, first: np.ndarray, second: np.ndarray,
    ...
    group_indices = {group: np.flatnonzero(np.asarray(groups) == group) for group in unique}
...
def main() -> None:
    ...
    summary = {
        "schema": "acie.official-fair-mlp-five-seed-summary.v1",
        "seeds": args.seeds,
        "aggregation": "arithmetic mean of per-seed probabilities",
        "directions": {},
    }

    for direction_index, direction in enumerate(DIRECTIONS):
        seed_paths = [official_root / direction / "mlp" / f"seed{seed}" / "predictions.jsonl" for seed in args.seeds]
        ids, labels, scores, metadata, score_matrix = aligned_ensemble(seed_paths)
        per_seed = []
        for seed, row_scores in zip(args.seeds, score_matrix, strict=True):
            metrics = json.loads((official_root / direction / "mlp" / f"seed{seed}" / "metrics.json").read_text(encoding="utf-8"))
            calculated = {"AP": float(average_precision_score(labels, row_scores)), "AUROC": float(roc_auc_score(labels, row_scores))}
            if not np.allclose([metrics["AP"], metrics["AUROC"]], [calculated["AP"], calculated["AUROC"]], rtol=0, atol=1e-12):
                raise ValueError(f"Metric mismatch for {direction} seed {seed}")
        ...
        ensemble_path = official_root / direction / "mlp" / "five_seed_ensemble_predictions.jsonl"
        ensemble_rows = []
        for sample_id, label, score, meta, seed_scores in zip(ids, labels, scores, metadata, score_matrix.T, strict=True):
            ensemble_rows.append({
                "sample_id": sample_id, "dataset": meta["dataset"], "recording": meta["recording"],
                "group_id": meta["group_id"], "track_id": meta["track_id"], "label": int(label),
                "score": float(score), "seed_scores": {str(seed): float(value) for seed, value in zip(args.seeds, seed_scores, strict=True)},
            })
        ensemble_path.write_text("".join(json.dumps(row, ensure_ascii=False) + "\n" for row in ensemble_rows), encoding="utf-8")

        comparisons = {}
        for method_index, method in enumerate(METHODS):
            acie_paths = [root / "outputs" / "frozen_cross_domain" / direction / method / f"seed{seed}" / "test_predictions.jsonl" for seed in args.seeds]
            acie_ids, acie_labels, acie_scores, _, _ = aligned_ensemble(acie_paths)
            if acie_ids != ids or not np.array_equal(acie_labels, labels):
                raise ValueError(f"Official/ACIE sample mismatch for {direction}/{method}")
            comparisons[f"official_mlp_minus_{method}"] = bootstrap_difference(
                labels, scores, acie_scores, [row["group_id"] for row in metadata],
                args.bootstrap, 20260914 + direction_index * 10 + method_index,
            )

        ap_values = [row["AP"] for row in per_seed]
        auc_values = [row["AUROC"] for row in per_seed]
        summary["directions"][direction] = {
            "n_test": len(ids),
            "test_positives": int(labels.sum()),
            "per_seed": per_seed,
            "single_seed": {
                "AP_mean": float(np.mean(ap_values)), "AP_std": float(np.std(ap_values, ddof=1)),
                "AUROC_mean": float(np.mean(auc_values)), "AUROC_std": float(np.std(auc_values, ddof=1)),
            },
            "ensemble": {
                "AP": float(average_precision_score(labels, scores)),
                "AUROC": float(roc_auc_score(labels, scores)),
                "predictions": str(ensemble_path.relative_to(root)),
                "predictions_sha256": sha256(ensemble_path),
            },
            "comparisons": comparisons,
        }

    output = official_root / "five_seed_summary.json"
