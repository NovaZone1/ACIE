...
def main():
 ...
 for d in DIRECTIONS:
  ...
  for sp in SPLITS:
   ...
   for m in models:
    aps=[]
    for seed in SEEDS:
     pred=read_jsonl(base/d/m/sp/str(seed)/'test_predictions.jsonl');aps.append(float(average_precision_score([r['label'] for r in pred],[r['score'] for r in pred])))
   ...
   pred=read_jsonl(base/d/'histgb'/sp/'11'/'test_predictions.jsonl');ap=float(average_precision_score([r['label'] for r in pred],[r['score'] for r in pred]));rows.append({'direction':d,'split':sp,'model':'histgb','AP_mean':ap,'AP_sample_SD':None,'seeds':1})
 ...
 csvout(base/'r3_split_table.csv',rows);csvout(base/'r3_difference_summary.csv',[{**x,'split_differences':json.dumps(x['split_differences']),'range':json.dumps(x['range'])} for x in summary]);write_json(base/'r3_summary.json',{'schema':'acie.vv-r3-summary.v1','per_split':rows,'differences':summary})
