#!/usr/bin/env python3
"""Validate the two-direction, five-seed official ST-GCN fair comparison."""
...
COMPARISONS = ["geometry", "dual_no_pair", "weighted_no_pair", "full_selected", "official_mlp", "official_lstm"]
DIRECTIONS = {
    "HUI360_to_SSUP-A": {"scope_files": ("hui_train_scope.json", "hui_val_scope.json", "ssup_target_scope.json"), "parts": ("train", "source_val", "target_test"), "epochs": 100},
    "SSUP-A_to_HUI360": {"scope_files": ("ssup_train_scope.json", "ssup_val_scope.json", "hui_target_scope.json"), "parts": ("train", "source_val", "target_test"), "epochs": 50},
}
...
def main() -> None:
    ...
    out = args.out or root / "validation" / "official_fair_stgcn_validation.json"
    protocol = load_json(root / "official_baselines" / "protocol_v1" / "PROTOCOL.json")
    summary = load_json(root / "outputs" / "official_baselines" / "fair_v1" / "five_seed_stgcn_summary.json")
    checks = []
    errors = []
    ...
    check("summary identity", summary.get("schema") == "acie.official-fair-stgcn-five-seed-summary.v1" and summary.get("model") == "stgcn" and summary.get("seeds") == SEEDS, {key: summary.get(key) for key in ("schema", "model", "seeds")})
    total_predictions = 0
    for direction, spec in DIRECTIONS.items():
        model_root = root / "outputs" / "official_baselines" / "fair_v1" / direction / "stgcn"
        target = protocol["directions"][direction]["fair"]["target_test"]
        train_config = yaml.safe_load((root / "official_baselines" / "protocol_v1" / direction / "fair_train_stgcn.yaml").read_text(encoding="utf-8"))
        target_config = yaml.safe_load((root / "official_baselines" / "protocol_v1" / direction / "fair_target_eval_stgcn.yaml").read_text(encoding="utf-8"))
        model_spec = protocol["model_specs"]["stgcn"][direction]
        config_valid = all(config["force_model_type"] == "stgcn" and config["epochs"] == spec["epochs"] and config["lr_scheduler_type"] == "none" and config["normalize_keypoints_in_box"] is False and config["standardize_data"] == "none" and config["stgcn_in_channels"] == 3 and config["stgcn_layout"] == "openpose" and config["stgcn_edge_importance_weighting"] is True for config in (train_config, target_config)) and model_spec["epochs"] == spec["epochs"]
        check(f"{direction}: official ST-GCN config", config_valid, {"epochs": train_config.get("epochs"), "lr_scheduler_type": train_config.get("lr_scheduler_type"), "normalize_keypoints_in_box": train_config.get("normalize_keypoints_in_box"), "standardize_data": train_config.get("standardize_data"), "in_channels": train_config.get("stgcn_in_channels"), "layout": train_config.get("stgcn_layout")})
        ...
        for seed in SEEDS:
            ...
            check(f"{direction}/seed{seed}: identity", metrics.get("schema") == "acie.official-fair-result.v1" and metrics.get("direction") == direction and metrics.get("model") == "stgcn" and metrics.get("seed") == seed, {key: metrics.get(key) for key in ("schema", "direction", "model", "seed")})
            ...
            check(f"{direction}/seed{seed}: provenance", provenance.get("schema") == "acie.official-seed-run.v1" and provenance.get("direction") == direction and provenance.get("model") == "stgcn" and provenance.get("seed") == seed and provenance.get("target_labels_used_for_selection") is False, {key: provenance.get(key) for key in ("direction", "model", "seed", "seed_control")})
        ...
        for comparison in COMPARISONS:
            if comparison.startswith("official_"):
                official_model = comparison.removeprefix("official_")
                paths = [root / "outputs" / "official_baselines" / "fair_v1" / direction / official_model / f"seed{seed}" / "predictions.jsonl" for seed in SEEDS]
            else:
            ...
            recorded = direction_summary["comparisons"][f"official_stgcn_minus_{comparison}"]["point"]
    ...
    report = {
        "schema": "acie.official-fair-stgcn-five-seed-validation.v1",
